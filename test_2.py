...
def login_check(user, password, url, block):
...
def test_step2(browser):
    ...
    testpage.click_login_button()
    time.sleep(2)
    assert f"{name}" in login_check(name, password, url_token, 'username'), "Test_2 FAIL"
...
def test_step4(browser, login):
    ...
    testpage.enter_name("Jorik")
    testpage.enter_email("moyou@mail.ru")
    testpage.enter_contact_content("Nu ti i molodec")
